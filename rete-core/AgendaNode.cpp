...
void AgendaNode::activate(Token::Ptr token, PropagationFlag flag)
{
    if (flag == rete::ASSERT)
    {
        ...
        bool removed = agenda_->removeEquivalent(AgendaItem{token, production_, rete::RETRACT, getName()});
        // only if no RETRACT was on the agenda we have to execute (and thus add to the agenda) an ASSERT
        if (!removed) agenda_->add(AgendaItem{token, production_, rete::ASSERT, getName()});
